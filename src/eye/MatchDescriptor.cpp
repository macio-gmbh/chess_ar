...
void MatchDescriptor::InitFrequence()
{
    ...
    if (nbElt == 0)
    {
        std::cout << "Contour is 0, abborting MatchDescriptor \n";
        return;
    }
...
void MatchDescriptor::falpha(float x, float *fn, float *df)
{
    ...
    for (n = 1; n <= nbDesFit; n++)
    {
        ...
        ds2 += -frequence[n] * frequence[n] * rho[n] * sin(psi[n] + frequence[n] * x) -
               frequence[nbElt - n] * frequence[nbElt - n] * rho[nbElt - n]
               * sin(psi[nbElt - n] + frequence[nbElt - n] * x);
        ds3 += -frequence[n] * rho[n] * sin(psi[n] + frequence[n] * x) -
               frequence[nbElt - n] * rho[nbElt - n] * sin(psi[nbElt - n] + frequence[nbElt - n] * x);
        ds4 += frequence[n] * frequence[n] * rho[n] * cos(psi[n] + frequence[n] * x) +
               frequence[nbElt - n] * frequence[nbElt - n] * rho[nbElt - n]
               * cos(psi[nbElt - n] + frequence[nbElt - n] * x);
    }
    *fn = s1 * s2 - s3 * s4;
...
Point2d MatchDescriptor::Echantillon(vector<Point> &c, long i, float l1, float l2, float s)
{
...
vector<Point2d> MatchDescriptor::ReSampleContour(vector<Point> &c, int nbElt)
{
    long nb = c.size();
    float l1 = 0, l2, p, d, s;
    vector<Point2d> r;
